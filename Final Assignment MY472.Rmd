...
Music, a ubiquitous presence in our lives, serves as a powerful cultural force. Understanding its evolution over time and unraveling the factors that contribute to the enduring impact of certain compositions is a compelling endeavor. This exploration seeks to dissect and analyze the elements that shape the longevity, influence, and broader impact of music.
...
4) **Wikipedia**: Extracts information about the nationality of artists and their total years of activity, using information from ‘Origin’, ‘Born’ and ‘Years Active’ in Wikipedia.
...
# STEP 9. WE CREATE THE DATA FROM OUR RELATIONAL DATABASE USING SQL -----------------------
...
It is worth noting that:
  ...
  * For some tables other variables such as **‘year’**, **‘decade’** and **‘origin’** will be added to give shape to the graphs.
...
The analysis demonstrates a positive relationship between the number of top tracks with high popularity (>75) and an artist's followers. This underscores the pivotal role that popular tracks play in expanding an artist's fan base, as they become ubiquitous and draw in new listeners.
...
Artists with Grammy Awards, the pinnacle of industry recognition, exhibit a strong correlation with high popularity. This correlation reinforces the notion that musical quality, coupled with industry accolades, significantly contributes to an artist's widespread appeal. 
...
Examining genre-specific album production unveils interesting trends, with rock dominating historically, and hip hop/rap steadily gaining ground. The popularity of artists across genres varies, with hip hop/rap taking the lead, followed by reggae, rock, soul/R&B, and pop. One interesting thing here is how Reggae happens to be so popular, when Bob Marley is the only artist in our database linked to this genre. This shows how influential can one’s legacy be.
...
Contrary to common assumptions, variables such as the number of singles produced, years of activity, and the release of 'live' albums show no significant, or negative correlation with overall artist popularity. This challenges preconceived notions about factors influencing an artist's widespread appeal.
...
live_albums_popularity_tracks <- plot_ly(
  ...
  layout(
    title = "Graph 13. Mean Popularity vs Total 'Live' Albums",
    xaxis = list(title = "Mean Artists' Popularity"),
...
As a complement to the analysis, some graphs are included below that include specific data for each artist, which may be of interest.
